// Cloudflare Worker for Visitor Counter
// Deploys to: https://api.ericmilan.dev or similar subdomain
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/counts/get' && request.method === 'GET') {
        const count = await env.VISITOR_COUNTER.get('count') || '0';
        return new Response(
      ...
      if (path === '/counts/increment' && request.method === 'POST') {
        // Get current count
        const currentCount = await env.VISITOR_COUNTER.get('count') || '0';
